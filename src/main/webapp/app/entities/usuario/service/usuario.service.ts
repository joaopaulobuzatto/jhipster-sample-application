...
type RestOf<T extends IUsuario | NewUsuario> = Omit<T, 'dataCriacao'> & {
...
export class UsuarioService {
  ...
  protected convertDateFromClient<T extends IUsuario | NewUsuario | PartialUpdateUsuario>(usuario: T): RestOf<T> {
    return {
      ...usuario,
      dataCriacao: usuario.dataCriacao?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsuario: RestUsuario): IUsuario {
    return {
      ...restUsuario,
      dataCriacao: restUsuario.dataCriacao ? dayjs(restUsuario.dataCriacao) : undefined,
    };
  }
